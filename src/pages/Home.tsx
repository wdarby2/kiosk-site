...
const Home: React.FC<HomeProps> = ({ onSpriteSelect }) => {
  ...
  // Set up auto-refresh every 30 minutes to prevent issues during long runtime
  useAutoRefresh({
    interval: 30 * 60 * 1000, // 30 minutes in milliseconds
    // interval: 3000, // 30 minutes in milliseconds
    enabled: true,
  });
      ...
      >
        ...
        >
          ...
          {sprites.length === 0 ? (
            ...
            <Grid sprites={sprites} onSpriteSelect={handleSpriteClick} />
          )}
        </div>{' '}
        {/* End of fixed-width-container */}
      </div>
    </div>
  );
};
